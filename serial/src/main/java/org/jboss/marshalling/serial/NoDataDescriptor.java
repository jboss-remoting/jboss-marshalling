 ...
 *
 */
class NoDataDescriptor extends Descriptor {

    NoDataDescriptor(final Class<?> type, final Descriptor parent) {
        super(parent, type);
    }
    ...
    protected void readSerial(final SerialUnmarshaller serialUnmarshaller, final SerializableClass sc, final Object subject) throws IOException, ClassNotFoundException {
        if (sc.hasReadObjectNoData()) {
            sc.callReadObjectNoData(subject);
        }
    }
}
