...
{
   public DerivedStreamingClass(Class<?> clazz) throws IOException {
      super(clazz);     
      ClassMetaData metadata = getMetadata();
      ClassMetaDataSlot[] slots = metadata.getSlots();
      short[][] keyfields = new short[slots.length][];
      for (int i = 0; i < slots.length; i++) {
         ClassMetaDataSlot slot = slots[i];
         short[] slotfields = new short[slot.getFields().length];
         for (int j = 0; j < slotfields.length; j++) {
            slotfields[j] = (short) j;
         }
         keyfields[i] = slotfields;
      }
