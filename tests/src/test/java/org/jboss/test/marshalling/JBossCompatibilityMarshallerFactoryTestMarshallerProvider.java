 ...
 *
 */
public class JBossCompatibilityMarshallerFactoryTestMarshallerProvider implements TestMarshallerProvider {
    private final MarshallerFactory marshallerFactory;
    private final int version;

    public JBossCompatibilityMarshallerFactoryTestMarshallerProvider(final MarshallerFactory factory) {
        marshallerFactory = factory;
        version = -1;
    }

    public JBossCompatibilityMarshallerFactoryTestMarshallerProvider(final MarshallerFactory factory, final int version) {
        marshallerFactory = factory;
    ...
    public Marshaller create(final MarshallingConfiguration config, final ByteOutput target) throws IOException {
       if (config.getClassExternalizerFactory() != null ||
        ...
        if (marshallerFactory instanceof JBossSerializationMarshallerFactory) {
           ((JBossSerializationMarshaller) marshaller).setJbossSerializationCompatible(true);
        }
